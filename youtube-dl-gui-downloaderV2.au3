...
#AutoIt3Wrapper_Res_File_Add=data\youtube-dl.exe
...
if $bFileInstall Then
	If Not FileExists("c:\tempYoutube-dl\") Then
		...
		FileInstall("data\youtube-dl.exe","C:\tempYoutube-dl\data\youtube-dl.exe")

	EndIf
	$picture = "c:\tempYoutube-dl\data\logo.JPG"
	$youtubedl = "c:\tempYoutube-dl\data\youtube-dl.exe"
	$icon = "c:\tempYoutube-dl\data\ico3.ico"


Else
	$picture = @ScriptDir & "\data\logo.JPG"
	$youtubedl = @ScriptDir & "\data\youtube-dl.exe"
...
While 1
	...
	Switch $nMsg
		...
		Case $mVerteiler
						...
						"" & @CRLF & _
						"" & @CRLF & _
						"dies ist eine GUI für youtube-dl.exe und wurde mit Autoit erstellt." & @CRLF & _
...
;~ 	$DOS = Run(@ComSpec & ' /c youtube-dl.exe  -ci --console-title '&$url, "", @SW_HIDE, $STDERR_CHILD + $STDOUT_CHILD)
...
;~ 		MsgBox($MB_SYSTEMMODAL, "Kontrolle", $command)
	...
	;~ 	$DOS = Run(@ComSpec & ' /c youtube-dl.exe --update', "", @SW_HIDE, $STDERR_CHILD + $STDOUT_CHILD)
	EndIf
EndFunc

func cleanup()
	$filePathNeu = "C:\tempYoutube-dl\data\youtube-dl.exe.new"
	$filePathAlt = "C:\tempYoutube-dl\data\youtube-dl.exe"
